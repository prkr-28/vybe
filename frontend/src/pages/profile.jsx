...
const Profile = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-black">
      {/* Header */}
      <div className="w-full h-14 bg-black flex items-center justify-between px-4 border-b border-gray-800 relative">
        {/* Back button */}
        <IoIosArrowBack
          onClick={() => navigate("/")}
          className="text-white text-2xl hover:text-blue-800 cursor-pointer z-10"
        />
        ...
        {/* Optional button on right */}
        {userData?.userName === profileData?.userName ? (
          <button
            onClick={handleLogOut}
            className="text-xs md:text-sm font-semibold px-[10px] py-[6px] min-w-[60px] bg-white hover:bg-gray-100 rounded-2xl cursor-pointer z-10"
          >
            Log Out
          </button>
        ) : (
      ...
      <div className="w-full h-[90px] flex flex-wrap items-center justify-center gap-16 md:gap-20 px-4 pt-8">
        ...
        <div className="text-center">
          ...
          <div className="text-gray-300 text-sm md:text-base">Following</div>
        </div>
      </div>
            ...
            (postData.filter((post) => post.author?.userName === userName)
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full mb-6">
                {postData
                  ...
                  .map((post) => (
                    <Post key={post._id} post={post} onProfile={true} />
                  ))}
              </div>
            ) : (
